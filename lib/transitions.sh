...
transitions::next() {
  ...
  case "$event" in
    UserPromptSubmit | PreToolUse | PostToolUse)
      ...
      ;;
    Notification)
      # User-attention notification kinds (Allow/Deny prompt, MCP elicitation
      # dialog, idle nudge) all mean "human input needed" and map to waiting.
      # Field has been named either `notification.kind` or `notification_type`
      # across releases; accept either, with kind taking precedence.
      local n_kind
