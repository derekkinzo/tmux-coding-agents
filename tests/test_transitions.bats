...
@test "Notification user-attention kinds → waiting (matrix: nested kind + flat type)" {
  # Every user-attention notification kind must produce 'waiting' regardless
  # of which field shape Claude sent (notification.kind nested vs.
  # notification_type flat). Asserted from current=working so the transition
  # is non-trivial (working → waiting catches a regression that simply
  # echoed current verbatim).
  for kind in permission_prompt idle_prompt elicitation_dialog; do
    payload="{\"notification\":{\"kind\":\"${kind}\"}}"
...
@test "Notification informational kinds preserve current state" {
  # auth_success, elicitation_complete, elicitation_response report state
  # transitions Claude already finished — they must NOT bump the pane to
  # waiting. Sanity-checked against an unknown 'xyz_future_kind' to verify
  # the default branch.
  for kind in auth_success elicitation_complete elicitation_response xyz_future_kind; do
    payload="{\"notification\":{\"kind\":\"${kind}\"}}"
    run transitions::next claude working Notification "$payload"
    assert_output "working"
  done
